...
/*---------------------------------------------------------------------------
 Name        : md5_string
 Input       : const char *input, char *output
 Output      : None
 Description :
 计算输入字符串的 MD5，并将结果输出到 output。

 设计背景：
 - 该函数意图用于预配置流程中 prov_code 的计算（示例：MD5(product_secret + device_sn)）。
 - 在真实产品中应使用标准 MD5 实现（输出通常为 16 字节二进制或 32 字节十六进制字符串）。

 当前实现状态：
 - 目前为占位实现（函数体为空），不会生成有效 MD5。
 - 工程中实际 prov_code 计算使用了 `MD5_Encryption()`（见 `at_provision_fetch()`），
   因此此函数在当前版本中不应被依赖。

 输入/输出约定（建议的未来实现）：
 - input：待计算的 C 字符串（以 '\\0' 结尾）
 - output：输出缓冲区，建议至少 33 字节用于存放 32 位 hex + '\\0'
---------------------------------------------------------------------------*/
