...
// 计算prov_code: 取MD5摘要字节4~11，转成16字符hex串
// 等同于ESP32的MD5_Encryption()
void MD5_Encryption(const char *input, char *result) {
    ...
    for (j = 0, i = 0; j < 16; j += 2, i++) {
        result[j]     = temp[i] & 0xf0;
        result[j]     = (result[j] >> 4) | 0x30;
        if (result[j] > '9')
            result[j] += 39;
        result[j + 1] = temp[i] & 0x0f;
        result[j + 1] = result[j + 1] | 0x30;
        if (result[j + 1] > '9')
            result[j + 1] += 39;
    }
    result[16] = '\0';
